...
var OtbAllocationService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.OtbAllocationService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../prisma/prisma.service");
let OtbAllocationService = OtbAllocationService_1 = class OtbAllocationService {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(OtbAllocationService_1.name);
        this.FACTOR_WEIGHTS = {
            historicalAvg: 0.4,
            sellThrough: 0.3,
            growthTrend: 0.2,
    ...
    async generateAllocation(input) {
        this.logger.log(`Generating allocation for budget detail: ${input.budgetDetailId}`);
        const warnings = [];
        const historicalData = await this.fetchHistoricalData(input);
        if (historicalData.length < 2) {
            warnings.push('Limited historical data available. Recommendations based on category averages.');
        }
        ...
        const overallConfidence = all.reduce((s, r) => s + r.confidence, 0) / (all.length || 1);
        const dataQuality = historicalData.length >= 3 ? 'high' : historicalData.length >= 2 ? 'medium' : 'low';
    ...
    async applyRecommendations(budgetDetailId, dimensionType) {
        const where = { budgetDetailId, isApplied: false };
        if (dimensionType)
    ...
    async compareAllocation(budgetDetailId, userAllocation) {
        ...
        const comparisons = userAllocation.map((user) => {
            const ai = recs.find((r) => r.dimensionType === user.dimensionType &&
        ...
        const avgDev = comparisons.reduce((s, c) => s + c.deviation, 0) /
    ...
    async fetchHistoricalData(input) {
        ...
        history.forEach((h) => {
            if (!byYear.has(h.fiscalYear))
                byYear.set(h.fiscalYear, []);
    ...
    async calcDimension(dimensionType, input, historicalData) {
        const values = await this.getDimensionValues(dimensionType);
        const recs = [];
        for (const value of values) {
            const valueHistory = historicalData
                .map((yd) => {
                const m = yd.data.find((d) => d.dimensionType === dimensionType && d.dimensionValue === value);
                return {
                    year: yd.year,
            ...
            const weighted = factors.historicalAvg * this.FACTOR_WEIGHTS.historicalAvg +
    ...
    calcFactors(history, dimensionType, value) {
        ...
        const avgST = history.reduce((s, h) => s + (h.sellThroughPct || 70), 0) /
    ...
    defaultPct(dimensionType, value) {
        const defaults = {
    ...
    async getDimensionValues(dimensionType) {
    ...
    genReasoning(factors, dataPoints, value) {
        const parts = [];
        if (dataPoints >= 3)
            parts.push('Based on 3 seasons of data.');
        else if (dataPoints >= 1)
            parts.push(`Based on ${dataPoints} season(s) of data.`);
        else
            parts.push('Using category default (no historical data).');
    ...
    async saveRecommendations(budgetDetailId, recs) {
        const detail = await this.prisma.budgetDetail.findUnique({
            where: { id: budgetDetailId },
        });
        if (!detail) {
            this.logger.warn(`BudgetDetail ${budgetDetailId} not found — skipping save. Recommendations still returned in response.`);
            return;
        }
...
exports.OtbAllocationService = OtbAllocationService;
exports.OtbAllocationService = OtbAllocationService = OtbAllocationService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], OtbAllocationService);
//# sourceMappingURL=otb-allocation.service.js.map