...
var SkuRecommenderService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.SkuRecommenderService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../prisma/prisma.service");
let SkuRecommenderService = SkuRecommenderService_1 = class SkuRecommenderService {
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new common_1.Logger(SkuRecommenderService_1.name);
        this.SCORE_WEIGHTS = {
            performance: 0.35,
            trend: 0.25,
            assortment: 0.25,
    ...
    async generateRecommendations(input) {
        this.logger.log(`Generating SKU recommendations for budget detail: ${input.budgetDetailId}, category: ${input.category}`);
        const warnings = [];
        ...
        const perfMap = await this.getPerformanceMap(eligibleSkus.map((s) => s.id), input.seasonGroup, input.fiscalYear);
        if (perfMap.size === 0) {
            warnings.push('No historical performance data available. Scoring based on catalog attributes only.');
        }
        const trendMap = await this.getAttributeTrends(input.category, input.seasonGroup, input.fiscalYear);
        if (trendMap.size === 0) {
            warnings.push('No attribute trend data available. Trend score uses defaults.');
        }
        const gaps = await this.analyzeAssortmentGaps(input.budgetDetailId, input.category, eligibleSkus);
        const scored = eligibleSkus.map((sku) => {
            ...
            const overallScore = Math.round(performanceScore * this.SCORE_WEIGHTS.performance +
            ...
            const reasoning = this.buildReasoning(sku, performanceScore, trendScore, assortmentScore, priceScore, riskLevel, perf);
            return {
                skuId: sku.id,
                skuCode: sku.skuCode,
        ...
        const selected = this.selectWithinBudget(scored.slice(0, maxResults * 2), input.budgetAmount, maxResults);
        ...
        const priceTiers = new Set(selected.map((s) => this.getPriceTier(s.srp)));
        const totalRecommendedValue = selected.reduce((sum, s) => sum + s.recommendedValue, 0);
        return {
    ...
    async getRecommendations(budgetDetailId, category) {
        const where = { budgetDetailId };
        if (category)
    ...
    async addSelectedToProposal(budgetDetailId, proposalId) {
        ...
        const totalOrderQty = allProducts.reduce((sum, p) => sum + p.orderQty, 0);
        const totalValue = allProducts.reduce((sum, p) => sum + Number(p.totalValue), 0);
    ...
    async getPerformanceMap(skuIds, seasonGroup, currentFiscalYear) {
    ...
    async getAttributeTrends(category, seasonGroup, currentFiscalYear) {
    ...
    async analyzeAssortmentGaps(budgetDetailId, category, eligibleSkus) {
        ...
        const existingPriceTiers = new Set();
        for (const product of existingProducts) {
            if (product.color)
                existingColors.add(product.color.toLowerCase());
            if (product.theme)
                existingThemes.add(product.theme.toLowerCase());
            const srp = Number(product.srp || 0);
            if (srp > 0)
                existingPriceTiers.add(this.getPriceTier(srp));
        }
        const allColors = new Set();
        const allThemes = new Set();
        const allPriceTiers = new Set();
        for (const sku of eligibleSkus) {
            if (sku.color)
                allColors.add(sku.color.toLowerCase());
            if (sku.theme)
                allThemes.add(sku.theme.toLowerCase());
            const srp = Number(sku.srp);
            if (srp > 0)
                allPriceTiers.add(this.getPriceTier(srp));
        }
        ...
        const missingThemes = new Set();
        for (const t of allThemes) {
            if (!existingThemes.has(t))
                missingThemes.add(t);
        }
        const missingPriceTiers = new Set();
        for (const p of allPriceTiers) {
            if (!existingPriceTiers.has(p))
                missingPriceTiers.add(p);
        }
        return {
            missingColors,
            missingThemes,
    ...
    calcPerformanceScore(perf) {
        ...
        return Math.round(sellThroughComponent + marginComponent + velocityComponent);
    }
    calcTrendScore(sku, trendMap) {
        let score = 50;
        let factors = 0;
        let totalScore = 0;
        if (sku.color) {
        ...
        const typeTrend = trendMap.get(`productType:${sku.productType.toLowerCase()}`);
        if (typeTrend) {
    ...
    calcPriceScore(sku, budgetAmount, totalEligible) {
        const srp = Number(sku.srp);
        if (srp <= 0)
            return 30;
    ...
    calcRiskLevel(perf, performanceScore) {
        if (!perf)
            return 'high';
        if (performanceScore >= 70 || perf.seasonsCount >= 2)
            return 'safe';
        if (performanceScore >= 50 || perf.seasonsCount >= 1)
            return 'moderate';
        return 'high';
    }
    selectWithinBudget(scored, budgetAmount, maxResults) {
        const selected = [];
        let runningTotal = 0;
    ...
    assignQuantities(selected, budgetAmount) {
        if (selected.length === 0)
            return;
    ...
    buildReasoning(sku, performanceScore, trendScore, assortmentScore, priceScore, riskLevel, perf) {
        const parts = [];
        if (perf) {
            if (performanceScore >= 70) {
                parts.push(`Strong performer (sell-through: ${perf.avgSellThrough.toFixed(1)}%, margin: ${perf.avgGrossMargin.toFixed(1)}%).`);
            }
            else if (performanceScore >= 50) {
                parts.push(`Moderate performer (sell-through: ${perf.avgSellThrough.toFixed(1)}%).`);
            }
            else {
    ...
    async saveRecommendations(input, items) {
    ...
    getPriceTier(srp) {
        if (srp < this.PRICE_TIERS.entry.max)
            return 'entry';
        if (srp < this.PRICE_TIERS.mid.max)
            return 'mid';
        if (srp < this.PRICE_TIERS.premium.max)
            return 'premium';
        return 'luxury';
    }
    emptyResult(input, warnings) {
        return {
            budgetDetailId: input.budgetDetailId,
            category: input.category,
            subCategory: input.subCategory,
...
exports.SkuRecommenderService = SkuRecommenderService;
exports.SkuRecommenderService = SkuRecommenderService = SkuRecommenderService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], SkuRecommenderService);
//# sourceMappingURL=sku-recommender.service.js.map