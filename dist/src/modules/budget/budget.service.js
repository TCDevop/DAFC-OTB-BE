...
let BudgetService = class BudgetService {
    ...
    async update(id, dto, userId) {
        ...
        if (!budget)
            throw new common_1.NotFoundException('Budget not found');
        if (budget.status !== enums_1.BudgetStatus.DRAFT) {
    ...
    async submit(id, userId) {
        ...
        return this.prisma.budget.update({
            where: { id },
            data: { status: enums_1.BudgetStatus.SUBMITTED },
        });
    }
    async remove(id) {
        const budget = await this.prisma.budget.findUnique({
    ...
    async approveLevel1(id, dto, userId) {
        ...
        if (!budget)
            throw new common_1.NotFoundException('Budget not found');
        if (budget.status !== enums_1.BudgetStatus.SUBMITTED) {
        ...
        const newStatus = dto.action === 'APPROVED'
            ? enums_1.BudgetStatus.LEVEL1_APPROVED
            : enums_1.BudgetStatus.REJECTED;
    ...
    async getStatistics(fiscalYear) {
        ...
        const approvedBudgets = await this.prisma.budget.aggregate({
            where: { ...where, status: enums_1.BudgetStatus.APPROVED },
            _sum: { totalBudget: true },
        });
        return {
            totalBudgets: total,
